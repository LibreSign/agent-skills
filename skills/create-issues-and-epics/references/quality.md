...
For `good first issue`, verify that prerequisites are available and the contract, affected area and tests are clear enough for a newcomer; the label reflects approachability, not merely short length. Do not label an entire complex epic just because a child is approachable. LibreSign examples such as [#8713](https://github.com/LibreSign/libresign/issues/8713) and [#8714](https://github.com/LibreSign/libresign/issues/8714) use a short `## Good first issue` section explaining where to start and what remains out of scope, followed by an `### Additional context` footer offering help and community links. When drafting for LibreSign, check the current issue template and recent issues, include those sections where they fit, and check the current URLs; for other repositories, use their own convention. Do not copy a long issue merely because it already carries the label: judge the entry path, prerequisite burden and reviewable scope independently. Use project-specific closing text, support links, templates and metadata only when the project uses them.
